...
/**
 * @author akshay Class That uses the Instance of the NODE BST class to insert
 *         values in all the Tress and The Observers.
 */
public class TreeBuilder {
	...
	public BST buildTree() {
		...
		while ((st = fp.readline()) != null) {
			Node node = tree.insert(Integer.parseInt(st.split(":")[0]),
					st.split(":")[1]);
			Node clone1 = cloneTree1In.insert(
					Integer.parseInt(st.split(":")[0]), st.split(":")[1]);
			Node clone2 = cloneTree2In.insert(
					Integer.parseInt(st.split(":")[0]), st.split(":")[1]);
